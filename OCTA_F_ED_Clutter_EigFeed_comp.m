...
function [p_tis,p_bld] = OCTA_F_ED_Clutter_EigFeed_comp(r_c, NumEV,quvT)
if size(r_c,3) ==1, p_tis = squeeze(r_c); p_bld = p_tis; return; end
if nargin < 3, quvT = 0;end
...
%     for ev=1:1:NumEV
    ...
    if quvT
        p_tis(:,i) = sum(abs(data*Pkx'),2)./Num_Ensembles;%% eig quv Real number
    else
        p_tis(:,i) = sum(data*Pkx',2)./(Num_Ensembles);% complex eig        
    end
