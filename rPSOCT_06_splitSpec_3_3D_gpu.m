function rPSOCT_06_splitSpec_3_3D_gpu(varargin)
% 这个函数使用GPU加速处理PSOCT数据
% 
% 用法:
%   rPSOCT_06_splitSpec_3_3D_gpu('参数名', 参数值, ...)
%
% 基于rPSOCT_06_splitSpec_3_3D_without_mat_wyx.m的GPU加速版本
%
% 作者: [Yongxin Wang]
% 日期: [2023]
...
%% 设置各种处理参数
hasSeg = false;
% 明确加载DispComp.mat文件中的变量
dispcomp_data = load('DispComp.mat');
phV = dispcomp_data.phV; % 确保变量名与文件中的一致

% 窗口长度（注：这个变量在后续代码中会使用）
winLen_val = 300;

% 所需B扫描的数量（直接使用nR，不再创建额外变量）
Avnum = 5; % averaging number along z
...
for iY=1:nY-1 % Bscan number
    ...
    fid=fopen(filename);
    fseek(fid,bob+(Blength*nX*2+2)*2*(nR*(iY-1)),'bof');
    for Ic=1:nR
        fseek(fid,4,'cof');  % 4byte =2 int16
        B=double(reshape(fread(fid,Blength*nX*2,'int16'),[Blength,nX*2]));
    ...
    if ~do_ssdopu % if not do adaptive gaussian filter dopu_ss =1 
        ...
        Bimg1 = zeros(Blength,nX,nR,nWin);Bimg2 = Bimg1;
        ...
        for iR = 1:nR
            for iL=1:nWin
                ...
                Bimg1(:,:,iR,iL)=fft(iBd1,Blength,1);
                Bimg2(:,:,iR,iL)=fft(iBd2,Blength,1);
            end
        end
    ...
    Bimg1_wholeStr=fft(Bd1.*gpu_winG_whole,Blength,1);Bimg2_wholeStr=fft(Bd2.*gpu_winG_whole,Blength,1);
...
if saveDicom
    ...
    for j=1:size(Strus,3)
        SS1=Strus(:,:,j);
        Struc(:,:,:,j)=(SS1-strLrg)./(strUrg-strLrg);
    end
    ...
    if do_cfg1
        ...
        if do_avg
            ...
            for jY =1:nY
                PRRc(:,:,:,jY) = uint8(ind2rgb(uint8(mat2gray(PhR_c_cfg1_avg(:,:,jY),PRRrg)*256),parula(256))*256);
                [cumLA_cfg_hsv(:,:,:,jY)] = quColoring(cumLA_cfg1_avg(:,:,:,jY),rotAngle);
                [LA_cfg_hsv(:,:,:,jY)] = quColoring(LA_c_cfg1_avg(:,:,:,jY),rotAngle);
                
                PRRc_rmBG(:,:,:,jY) = uint8(ind2rgb(uint8(mat2gray(PhR_Ms_cfg1_rmBG(:,:,jY),PRRrg)*256),parula(256))*256);
                [cumLA_Ms_cfg1_rmBG_hsv(:,:,:,jY)] = quColoring(cumLA_Ms_cfg1_rmBG(:,:,:,jY),rotAngle);
                [LA_Ms_cfg1_rmBG_hsv(:,:,:,jY)] = quColoring(LA_Ms_cfg1_rmBG(:,:,:,jY),rotAngle);
            end
        ...
        if do_eig
            ...
            for jY =1:nY
                PRRc(:,:,:,jY) = uint8(ind2rgb(uint8(mat2gray(PhR_c_cfg1_eig(:,:,jY),PRRrg)*256),parula(256))*256);
                [cumLA_cfg_hsv(:,:,:,jY)] = quColoring(cumLA_cfg1_eig(:,:,:,jY),rotAngle);
                [LA_cfg_hsv(:,:,:,jY)] = quColoring(LA_c_cfg1_eig(:,:,:,jY),rotAngle);
            end
    ...
    if do_cfg2
        ...
        if do_avg
            ...
            for jY =1:nY
                PRRc(:,:,:,jY) = uint8(ind2rgb(uint8(mat2gray(PhR_c_cfg2_avg(:,:,jY),PRRrg)*256),parula(256))*256);
                [cumLA_cfg_hsv(:,:,:,jY)] = quColoring(cumLA_cfg2_avg(:,:,:,jY));
                [LA_cfg_hsv(:,:,:,jY)] = quColoring(LA_c_cfg2_avg(:,:,:,jY));
            end
        ...
        if do_eig
            ...
            for jY =1:nY
                PRRc(:,:,:,jY) = uint8(ind2rgb(uint8(mat2gray(PhR_c_cfg2_eig(:,:,jY),PRRrg)*256),parula(256))*256);
                [cumLA_cfg_hsv(:,:,:,jY)] = quColoring(cumLA_cfg2_eig(:,:,:,jY));
                [LA_cfg_hsv(:,:,:,jY)] = quColoring(LA_c_cfg2_eig(:,:,:,jY));
            end
...
end % function rPSOCT_06_splitSpec_3_3D_gpu

%% 辅助函数
function [rgb] = quColoring(qu,rotAngle)
if nargin < 2
    rotAngle = 0;
end
hsv = qu*0;
% QU+90
for k=1:size(qu,3)
    q=qu(:,:,1,k);u=qu(:,:,2,k);v=qu(:,:,3,k);
    hsv(:,:,1,k)=mod(atan2d(u,q)+rotAngle,360)/360;
    hsv(:,:,2,k)=sqrt(q.^2+u.^2+v.^2);
    hsv(:,:,3,k)=ones(size(q));
end
rgb = hsv2rgb(hsv);
end

function [vFlatten] = volFlatten(vol,surf)
vFlatten = vol;
surf = round(surf); % 确保表面坐标是整数
nY = size(vol,4);
nX = size(vol,2);
surfDiff = max(surf(:))-min(surf(:));

for jY = 1:nY
    for jX = 1:nX
        if surf(jX,jY) > 0
            vFlatten(:,jX,:,jY) = circshift(vol(:,jX,:,jY),-surf(jX,jY)+surfDiff+1,1);
        end
    end
end
end