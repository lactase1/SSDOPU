...
delete(gcp('nocreate'));
% 在代码开始处，clear all;close all;clc; 之后添加：
parpool('local', 19);      % 启动本地并行池;  % 使用基于进程的并行池
% 定义数据路径
data_path = 'D:\1-Liu Jian\yongxin.wang\data\';
...
% tmp=dir('.\*.oct');
tmp=dir(fullfile(data_path, '*.oct'));
for iFile=1:size(tmp,1), fprintf(['[%d]\t',tmp(iFile).name(1:end-4),'\n'],iFile);end
%% (2)choose file and select Z range
tic
do_cfg1 = 1;
do_cfg2 = 0;
do_avg = 1;
...
for iFile=1:1
    fprintf('\n==================================================\n');
    fprintf('正在处理第 %d/%d 个文件: %s\n', iFile, length(tmp), tmp(iFile).name);
    fprintf('==================================================\n');
    ...
    fid=fopen(fullfile(data_path, filename));
...
topLines =ones(nX,nY);
if hasSeg
    load(fullfile(data_path, [filename,'.mat']));
...
dicomwrite(uint8(255*(permute(dopu_splitSpectrum,[1 2 4 3]))),[foutputdir,filename,'_1-4_dopu_SS.dcm']);
